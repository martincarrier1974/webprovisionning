...
import { StorageStatusCard } from "@/components/admin/storage-status-card";
...
export default async function DashboardPage() {
  ...
  const storageSummary = getObjectStorageSummary();
  ...
  return (
    <main style={{ minHeight: "100vh", padding: "40px 24px 64px", background: "radial-gradient(circle at top, rgba(28, 100, 242, 0.12), transparent 32%), linear-gradient(180deg, #081120 0%, #0f172a 100%)", color: "#f8fafc" }}>
      <section style={{ maxWidth: 1120, margin: "0 auto", display: "grid", gap: 24 }}>
        ...
        <StorageStatusCard storage={storageSummary} />
        ...
        <section style={twoColSectionStyle}>
          <article style={panelStyle}><div style={{ marginBottom: 18 }}><p style={{ color: "#93c5fd", marginBottom: 8 }}>Clients récents</p><h2 style={{ fontSize: 24, marginBottom: 8 }}>Clients configurés</h2></div><div style={{ display: "grid", gap: 12 }}>{clients.length === 0 ? <p style={{ color: "#cbd5e1" }}>Aucun client pour le moment.</p> : clients.map((client) => <div key={client.id} style={itemCardStyle}><div style={{ fontWeight: 700 }}>{client.name}</div><div style={{ color: "#cbd5e1", marginTop: 4 }}>{client.slug}</div><div style={{ color: "#93c5fd", marginTop: 6, fontSize: 14 }}>{client._count.sites} site(s) · {client._count.phones} téléphone(s)</div><ClientManagementActions client={client} /></div>)}</div></article>
          <article style={panelStyle}><div style={{ marginBottom: 18 }}><p style={{ color: "#93c5fd", marginBottom: 8 }}>Sites récents</p><h2 style={{ fontSize: 24, marginBottom: 8 }}>Sites configurés</h2></div><div style={{ display: "grid", gap: 12 }}>{sites.length === 0 ? <p style={{ color: "#cbd5e1" }}>Aucun site pour le moment.</p> : sites.map((site) => <div key={site.id} style={itemCardStyle}><div style={{ fontWeight: 700 }}>{site.name}</div><div style={{ color: "#cbd5e1", marginTop: 4 }}>{site.client.name} · {site.slug}</div><div style={{ color: "#93c5fd", marginTop: 6, fontSize: 14 }}>{site._count.phones} téléphone(s)</div><SiteManagementActions site={{ id: site.id, clientId: site.client.id, name: site.name, slug: site.slug, address: site.address, city: site.city, province: site.province, country: site.country, timezone: site.timezone }} clients={clientOptions.map(({ id, name }) => ({ id, name }))} /></div>)}</div></article>
        </section>

        <section style={twoColSectionStyle}>
          <article style={panelStyle}><div style={{ marginBottom: 18 }}><p style={{ color: "#93c5fd", marginBottom: 8 }}>Téléphones récents</p><h2 style={{ fontSize: 24, marginBottom: 8 }}>Téléphones configurés</h2></div><div style={{ display: "grid", gap: 12 }}>{phones.length === 0 ? <p style={{ color: "#cbd5e1" }}>Aucun téléphone pour le moment.</p> : phones.map((phone) => <div key={phone.id} style={itemCardStyle}><div style={{ fontWeight: 700 }}>{phone.label || phone.macAddress}</div><div style={{ color: "#cbd5e1", marginTop: 4 }}>{phone.phoneModel.vendor} · {phone.phoneModel.displayName}</div><div style={{ color: "#cbd5e1", marginTop: 4 }}>{phone.client.name}{phone.site ? ` · ${phone.site.name}` : ""}</div><div style={{ color: "#93c5e1", marginTop: 6, fontSize: 14 }}>MAC {phone.macAddress} · {phone.status}</div><EditPhoneForm phone={{ id: phone.id, clientId: phone.client.id, siteId: phone.site?.id ?? null, phoneModelId: phone.phoneModel.id, macAddress: phone.macAddress, label: phone.label, extensionNumber: phone.extensionNumber, sipUsername: phone.sipUsername, sipPassword: phone.sipPassword, sipServer: phone.sipServer, webPassword: phone.webPassword, adminPassword: phone.adminPassword, status: phone.status }} clients={clientOptions.map(({ id, name }) => ({ id, name }))} sites={siteOptions.map(({ id, name }) => ({ id, name }))} phoneModels={phoneModelOptions.map(({ id, displayName, vendor }) => ({ id, displayName, vendor }))} /></div>)}</div></article>
          <article style={panelStyle}><div style={{ marginBottom: 18 }}><p style={{ color: "#93c5fd", marginBottom: 8 }}>Firmwares</p><h2 style={{ fontSize: 24, marginBottom: 8 }}>Firmwares configurés</h2></div><div style={{ display: "grid", gap: 12 }}>{firmwares.length === 0 ? <p style={{ color: "#cbd5e1" }}>Aucun firmware pour le moment.</p> : firmwares.map((firmware) => <div key={firmware.id} style={itemCardStyle}><div style={{ fontWeight: 700 }}>{firmware.phoneModel.vendor} · {firmware.phoneModel.displayName}</div><div style={{ color: "#cbd5e1", marginTop: 4 }}>v{firmware.version} · {firmware.originalName}</div><div style={{ color: "#93c5fd", marginTop: 6, fontSize: 14 }}>{firmware.status} · {firmware._count.phones} téléphone(s){firmware.isDefault ? " · défaut" : ""}</div><FirmwareManagementActions firmware={{ id: firmware.id, phoneModelId: firmware.phoneModel.id, version: firmware.version, storageKey: firmware.storageKey, originalName: firmware.originalName, checksumSha256: firmware.checksumSha256, releaseNotes: firmware.releaseNotes, status: firmware.status, isDefault: firmware.isDefault }} /></div>)}</div></article>
        </section>

        <section style={twoColSectionStyle}>
          <article style={panelStyle}><div style={{ marginBottom: 18 }}><p style={{ color: "#93c5fd", marginBottom: 8 }}>Règles de provisioning</p><h2 style={{ fontSize: 24, marginBottom: 8 }}>Hiérarchie résolue</h2></div><div style={{ display: "grid", gap: 12 }}>{provisioningRules.length === 0 ? <p style={{ color: "#cbd5e1" }}>Aucune règle pour le moment.</p> : provisioningRules.map((rule) => <div key={rule.id} style={itemCardStyle}><div style={{ fontWeight: 700 }}>{rule.source} · {rule.key}</div><div style={{ color: "#cbd5e1", marginTop: 4 }}>{rule.value}</div><div style={{ color: "#93c5fd", marginTop: 6, fontSize: 14 }}>ordre {rule.sortOrder}{rule.client ? ` · client ${rule.client.name}` : ""}{rule.site ? ` · site ${rule.site.name}` : ""}{rule.phoneModel ? ` · modèle ${rule.phoneModel.displayName}` : ""}{rule.phone ? ` · téléphone ${rule.phone.label || rule.phone.macAddress}` : ""}</div></div>)}</div></article>
          <article style={panelStyle}><div style={{ marginBottom: 18 }}><p style={{ color: "#93c5fd", marginBottom: 8 }}>Derniers comptes</p><h2 style={{ fontSize: 24, marginBottom: 8 }}>Utilisateurs récents</h2></div><div style={{ display: "grid", gap: 12 }}>{users.length === 0 ? <p style={{ color: "#cbd5e1" }}>Aucun utilisateur disponible pour le moment.</p> : users.map((item) => <div key={item.id} style={itemCardStyle}><div style={{ fontWeight: 700 }}>{[item.firstName, item.lastName].filter(Boolean).join(" ") || item.email}</div><div style={{ color: "#cbd5e1", marginTop: 4 }}>{item.email}</div><div style={{ color: "#93c5fd", marginTop: 6, fontSize: 14 }}>{item.role} · {item.status}</div><UserManagementActions userId={item.id} status={item.status} isCurrentUser={item.id === user.id} /></div>)}</div></article>
        </section>
      </section>
    </main>
  );
