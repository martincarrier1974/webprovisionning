...
export async function POST(
  ...
  const phoneRecord = await db.phone.findUnique({ where: { id }, select: { macAddress: true } });
  ...
  await db.provisionLog.create({
    data: {
      ...
      success: true,
      statusCode: 200,
      message: `Resync manuel — ${resolved.resolvedEntries.length} règles`,
    },
  });

  return NextResponse.json({ ok: true, linesGenerated: config.split("\n").length });
}
